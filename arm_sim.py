...
#     def plot(self, joint_angles_deg):
#         # 1. 计算所有坐标系矩阵
#         transforms = [np.eye(4)]
#         current_T = np.eye(4)
        
#         for i, row in enumerate(self.dh_table):
#             # 实际 theta = 控制输入角度 + DH表中的初始偏移
#             theta_total = np.radians(joint_angles_deg[i]) + row[0]
...
#         ax.set_title("6-DOF Arm ($\theta, d, \\alpha, a$ Order)")
...
# def main():
#     # 按照你提供的参数重新排列为 [theta_offset, d, alpha, a]
#     dh_params = np.array([
#         # [theta_off,  d,         alpha,     a  ]
...
#     # 🔥 在这里调节 6 个关节的实时转动角度 (从初始姿态开始转) 单位：角度
#     target_angles = [np.pi, 0, 0, 0, 0, 0]
#     # target_angles = [-2.0574, 0.0745, 1.7437, -1.6996, 0.4705, 1.2924]
...
class StandardDHArm:
    ...
    def extract_pose(self, T):
        ...
        # 这里为了终端显示直观，末端姿态的输出依然保留转成角度 (degrees)
        # return x, y, z, np.degrees(roll), np.degrees(pitch), np.degrees(yaw)
    ...
    def plot(self, joint_angles_rad):
        ...
        for i, row in enumerate(self.dh_table):
            # 🔥 核心修改：不再使用 np.radians()，直接把输入的弧度值加上 offset
